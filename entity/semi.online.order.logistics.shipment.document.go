...
type SemiOnlineOrderLogisticsShipmentDocument struct {
	...
	// 使用如下方式访问
	// 访问地址：这里返回的 url，url 10分钟后失效，超时请重新调用 bg.logistics.shipment.document.get 获得新的 url
	// 请求方式：GET
	// 请求header包含以下5个参数：
	// {
	//	"toa-app-key": ${app_key},
	...
	//	"toa-sign":"${sign}"
	// }
	// 其中 toa-random 自定义32个随机数
	// 其中 toa-timestamp 长度要求 10 位，精确到秒级，current_time-300<=toa-timestamp<=current_time+300
	// 其中 toa-sign 的计算逻辑与正常请求里公共参数 sign 的计算逻辑一致：
	// - 将 toa-access-token 和toa-app-key 和 toa-random和toa-timestamp 4个参数进行首字母以ASCII方式升序排列 ascii asc，对于相同字母则使用下个字母做二次排序，字母序为从左到右，以此类推
	// - 排序后的结果按照参数名 $key 参数值 $value 的次序进行字符串拼接，拼接处不包含任何字符
	// - 拼接完成的字符串做进一步拼接成1个字符串（包含所有kv字符串的长串），并在该长串的头部及尾部分别拼接 app_secret，完成签名字符串的组装
