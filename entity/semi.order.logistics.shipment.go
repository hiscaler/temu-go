...
type ShipmentInfo struct {
	...
	Quantity             int                   `json:"quantity"`                   // 商品 skuId 对应发货数量
	...
	CooperativeWarehouse *CooperativeWarehouse `json:"cooperativeWarehouseDTO"`    // 合作对接仓信息，仅在 packageDeliveryType=3 或 4 时返回
	TrackingWarningLabel int                   `json:"trackingWarningLabel"`       // 运单物流提醒标签：0-无问题 1-查无轨迹 2-疑似有误 3-收货地址不一致 4-未揽收
	SubPackageShipments  []SubPackageShipment  `json:"subPackageShipmentInfoList"` // 附属包裹列表，当为单 sku 拆单发货场景时，后续增加补充的运单信息将作为附属包裹展示
}
