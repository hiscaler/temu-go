...
type ParentOrder struct {
	...
	} `json:"parentOrderLabel"`                                                 // PO 单订单状态标签
...
type ChildOrder struct {
	...
	Quantity                          int    `json:"quantity"`                          // 单应履约件数 备注：代表商家实际需要发货件数，在订单部分取消时：应履约件数=下单件数-发货前售后件数
	...
	OrderLabel                        []struct {
		Name  string `json:"name"`  // 标签名称，具体枚举如下 customized_products：定制品标签		US_to_CA：美发加订单标签
		Value int    `json:"value"` // 是否有标签：0=无标签，1=有标签
	} `json:"orderLabel"`                           // 子订单O单标签，内部请求异常返回为空，返回为空时请重试
	Spec            string `json:"spec"`            // 商品信息描述
	ThumbUrl        string `json:"thumbUrl"`        // 商品缩略图图片
	OrderStatus     int    `json:"orderStatus"`     // 订单状态，3是已取消
