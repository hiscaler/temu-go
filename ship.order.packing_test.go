...
func TestShipOrderPackingService_Send(t *testing.T) {
	...
	assert.Nilf(t, err, "temuClient.Services.LogisticsService.Companies(): error")
	assert.Equal(t, true, len(companies) > 0, "temuClient.Services.LogisticsService.Companies(): results")
	company := companies[0]
